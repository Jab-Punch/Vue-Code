<template>
    <div>
        ...
        <button @click="addCpt()" class="text-button no-bg weight-600">
            <span class="material-icons purple">add_box</span> Add additional custom service
        </button>
    </div>
</template>
        ...
        methods: {
            async init() {
                ...
                this.service_fees = res.data
                    .filter(
                        (fee) => fee.level === 'client' && fee.level_id == this.client_id && fee.code_type == 'product'
                    )
                    .map((element) => {
                        element.selected = {
            ...
            async getServiceCodeFeeOptions() {
                ...
                this.options = result.data.filter((fee) => fee.level === 'company' && fee.code_type == 'product');
